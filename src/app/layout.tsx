...
export const metadata: Metadata = {
  title: "Coffee Shop - A Brew Above",
  description: "Experience the best coffee in town. Discover our unique blends and cozy ambiance.",
  openGraph: {
    title: "Coffee Shop - A Brew Above",
    description: "Experience the best coffee in town. Discover our unique blends and cozy ambiance.",
    url: "",
    siteName: "",
    images: [{"url":"asset://hero-image","alt":"Coffee Shop Interior"}],
    type: "website"
  },
  twitter: {
...
(function() {
  ...
  const getElementType = (element) => {
    ...
    // Block elements with data-section
    if (element.hasAttribute('data-section')) return 'Block';

    // Text elements
    const textTags = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'span', 'label'];
    if (textTags.includes(tagName)) return 'Text';

    // If element is editable text
    if (isTextElement(element)) return 'Text';

    // Return null for unclassifiable elements
    return null;
  };
  ...
  const isValidElement = (element) => {
    ...
    // Filter out elements that cannot be classified
    const elementType = getElementType(element);
